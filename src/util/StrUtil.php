...
class StrUtil
{
     ...
     * @param string|bool $forceValueType 强制转换值类型
     * @return array
     */
    public static function ToArray($str = '', $glue = ',', $forceValueType = false)
    {
        if (!empty($str)) {
            $arr = [];
            if (strpos($str, $glue)) {
            ...
            //强制转换值类型
            if ($forceValueType && !empty($forceValueType)) {
                foreach ($arr as &$item) {
                    settype($item, $forceValueType);
                }
            }
            return $arr;
        } else {
     ...
     * @return string
     */
    public static function Sub($str, $len = 100, $start = 0, $encoding = 'UTF-8', $suffix = '')
    {
        $str = self::RemoveHtml($str);
        $strlen = mb_strlen($str, $encoding);
        if ($strlen > $len) {
            $str = mb_substr($str, $start, $len, $encoding) . $suffix;
        } else if ($start > 0 && $strlen > $start) {
            $str = mb_substr($str, $start, $strlen, $encoding) . $suffix;
        }
        return $str;
    }
     ...
     *
     * @param string $str
     * @param integer $star 隐藏长度
     * @param string $postion 位置
     * @return string
     */
    public static function Hide($str, $star = 4, $postion = 'center')
    {
        if (empty($str)) return '';
        $len = mb_strlen($str);
        $length = $star;
        ...
        $starStr = str_repeat('*', $length);
        if ($len == 3) $length -= 1;
        if ($postion == 'left') {
            return $starStr . mb_substr($str, $length * (-1));
        } else if ($postion == 'right') {
            return mb_substr($str, 0, $length) . $starStr;
        }
        $r = $len - $length;
